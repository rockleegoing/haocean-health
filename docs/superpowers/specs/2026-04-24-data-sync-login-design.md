...
│  └── 媒体文件（media_file，单文件限 20MB，有网即下）          │
│                                                             │
│  暂不纳入（功能未完成）：                                     │
│  ├── 单位信息（执法对象）— 预留同步方法，待后端开发            │
│                                                             │
│  网络策略：只验证有网                                         │
...
| 行业分类 | 后端 API | 无 | 获取全量分类数据 |
| 法律法规库 | 后端 API | 20MB | 支持增量（lastModified） |
| 规范用语库 | 后端 API | 20MB | 支持增量（lastModified） |
| 监管事项库 | 后端 API | 20MB | 支持增量（lastModified） |
| 文书模板 | 后端 API | 20MB | 支持增量（lastModified） |
| 媒体文件 | 后端 API | 20MB | 支持增量（lastModified） |
...
### 9.2 阶段二/三：各模块数据接口（预留实现）

| 模块 | 接口路径 | 参数 | 说明 |
|------|----------|------|------|
| 行业分类 | GET /industry/category/list | - | 全量获取 |
...
|--------|------|------|
| P0 | 阶段一：登录页数据预加载 | - |
| P0 | 阶段二：全量阻塞式同步 | 后端 /getInfo |
| P1 | 阶段三：WorkManager 后台同步 | 后端增量接口 |
| P2 | 媒体文件分片下载 + MD5 校验 | 后端接口 |
| P3 | 覆盖安装检测和清空逻辑 | - |
| P3 | 新增表实体和 DAO | - |
